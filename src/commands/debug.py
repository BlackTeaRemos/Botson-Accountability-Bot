...
def RegisterDebugCommands(
    ...
    @debug_group.command(name="delete_all_reports", description="Delete all reports from the database")
    async def DebugDeleteAllReports(interaction: discord.Interaction):
        try:
            await interaction.response.defer(ephemeral=True, thinking=True)
    ...
    @debug_group.command(name="delete_test_users", description="Delete all test user data from this channel")
    async def DebugDeleteTestUsers(interaction: discord.Interaction):
        try:
            cid = interaction.channel_id
            if cid is None:
                await interaction.response.send_message("This command must be used in a channel.", ephemeral=True)
    ...
    @debug_group.command(name="delete_all_user_data", description="⚠️ DELETE ALL user data from this channel (destructive!)")
    async def DebugDeleteAllUserData(interaction: discord.Interaction):
        try:
            cid = interaction.channel_id
            if cid is None:
                await interaction.response.send_message("This command must be used in a channel.", ephemeral=True)
            ...
            if total_deleted == 0:
                ...
                await interaction.followup.send(
                    f"⚠️ **WARNING: ALL USER DATA DELETED**\n\n"
    ...
    _registered_debug_commands: dict[str, object] = {
        ...
        "delete_all_reports": DebugDeleteAllReports,
        "delete_test_users": DebugDeleteTestUsers,
        "delete_all_user_data": DebugDeleteAllUserData,
    }
